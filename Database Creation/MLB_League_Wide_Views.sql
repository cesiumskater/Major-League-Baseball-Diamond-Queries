-- =====================================================================
-- MAJOR LEAGUE BASEBALL - 100 DATABASE VIEWS
-- Team-specific analytics for the Major League Baseball (teamID IS NOT NULL)
-- Franchise established: 1871
-- =====================================================================


-- ============================================================
-- View 1: mlb_season_results
-- Purpose: Year-by-year W/L record with postseason flags and
--          run differential for every MLB Teams season.
-- Key formulas: W%, run_diff = R - RA,
--   Pyth W% = R^2 / (R^2 + RA^2)
-- Qualifiers: teamID IS NOT NULL
-- ============================================================

CREATE OR REPLACE VIEW mlb_season_results AS
SELECT
    t.yearID,
    t.lgID,
    t.divID,
...
-- ============================================================
-- View 2: mlb_decade_summary
-- Purpose: MLB Teams performance aggregated by decade.
-- Key formulas: W%, total R, total RA, run differential,
--   division titles, playoff appearances per decade.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_decade_summary AS
SELECT
...
-- ============================================================
-- View 3: mlb_all_time_roster
-- Purpose: Every player who appeared for the MLB Teams with
...
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_all_time_roster AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 4: mlb_managers_record
-- Purpose: All MLB Teams managers with win/loss record and
--          seasons managed.
-- Key formulas: W%, years managed.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_managers_record AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 6: mlb_home_parks
-- Purpose: Home stadiums/parks used by the MLB Teams over time.
-- Key formulas: Games and attendance by park.
-- Qualifiers: teamkey IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_home_parks AS
SELECT
...
WHERE hg.teamkey IS NOT NULL
ORDER BY hg.yearkey, pk.parkname;

-- ============================================================
-- View 7: mlb_run_differential
...
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_run_differential AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 8: mlb_al_west_standings
-- Purpose: MLB Teams standing within the AL West each year,
--          compared to division rivals.
-- Key formulas: W%, team rank within division.
-- Qualifiers: AL West teams (divID IS NOT NULL, lgID IS NOT NULL)
-- Notes: Shows full AL West standings for context.
-- ============================================================
CREATE OR REPLACE VIEW mlb_al_west_standings AS
SELECT
    t.yearID,
    t.teamID,
    t.name                                                     AS team_name,
    ...
    t.DivWin,
    t.WCWin,
    CASE WHEN t.teamID IS NOT NULL THEN 'Y' ELSE 'N' END         AS is_active_franchise
FROM teams t
WHERE t.lgID IS NOT NULL
  AND t.divID IS NOT NULL
  AND t.yearID >= 1871
ORDER BY t.yearID DESC, t.teamRank ASC;

-- ============================================================
-- View 9: mlb_postseason_years
-- Purpose: Summary of every MLB Teams postseason appearance.
-- Key formulas: W%, postseason flags from teams table.
-- Qualifiers: teamID IS NOT NULL, made postseason
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_years AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 10: mlb_franchise_milestones
...
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_franchise_milestones AS
SELECT
...
-- ============================================================
-- View 11: mlb_career_batting_standard
-- Purpose: Career batting totals for all MLB Teams hitters
--          (only counting MLB Teams stints), with standard
--          rate stats AVG/OBP/SLG/OPS.
-- Key formulas: Standard rate stats.
-- Qualifiers: MLB Teams stints only (teamID IS NOT NULL), AB >= 100
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 12: mlb_career_batting_advanced
-- Purpose: Career advanced batting metrics for MLB Teams hitters:
--          ISO, BABIP, wOBA, BB%, K%.
-- Key formulas: ISO = SLG - AVG, BABIP = (H-HR)/(AB-SO-HR+SF),
--   wOBA with linear weights, BB% = BB/PA, K% = SO/PA
-- Qualifiers: teamID IS NOT NULL, PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 13: mlb_career_batting_counting_leaders
-- Purpose: Career counting stat leaders for MLB Teams:
--          HR, RBI, R, H, SB with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_batting_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 14: mlb_career_ops_leaders
-- Purpose: Top career OPS among MLB Teams hitters with minimum
--          1000 PA in MLB.
-- Key formulas: OBP + SLG = OPS.
-- Qualifiers: teamID IS NOT NULL, PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 15: mlb_career_batting_war_approx
-- Purpose: Approximate career WAR for MLB Teams position players
...
-- Qualifiers: teamID IS NOT NULL, career PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_batting_war_approx AS
WITH sea_batting AS (
...
-- ============================================================
-- View 16: mlb_career_batting_by_position
-- Purpose: Aggregate batting performance by primary position
--          for MLB Teams players across franchise history.
-- Key formulas: AVG, OBP, SLG, OPS by position.
-- Qualifiers: teamID IS NOT NULL, PA >= 200 per player
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_batting_by_position AS
WITH sea_pos AS (
    SELECT a.playerID,
        CASE
            WHEN SUM(COALESCE(a.G_c, 0)) >= GREATEST(SUM(COALESCE(a.G_1b, 0)), SUM(COALESCE(a.G_2b, 0)), SUM(COALESCE(a.G_3b, 0)), SUM(COALESCE(a.G_ss, 0)), SUM(COALESCE(a.G_lf, 0)), SUM(COALESCE(a.G_cf, 0)), SUM(COALESCE(a.G_rf, 0)), SUM(COALESCE(a.G_dh, 0))) THEN 'C'
...
-- ============================================================
-- View 17: mlb_100_hr_club
-- Purpose: MLB Teams players who hit 100+ HR in their MLB Teams
--          career.
-- Key formulas: SUM(HR), career rate stats.
-- Qualifiers: teamID IS NOT NULL, career HR >= 100
-- ============================================================
CREATE OR REPLACE VIEW mlb_100_hr_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 18: mlb_1000_hit_club
-- Purpose: MLB Teams players who recorded 1000+ hits in MLB.
-- Key formulas: SUM(H), career rate stats.
-- Qualifiers: teamID IS NOT NULL, career H >= 1000
-- ============================================================
CREATE OR REPLACE VIEW mlb_1000_hit_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 19: mlb_career_obp_leaders
-- Purpose: Top career OBP among MLB Teams hitters (min 1000 PA).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID IS NOT NULL, PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_obp_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 20: mlb_career_slg_leaders
-- Purpose: Top career SLG among MLB Teams hitters (min 1000 PA).
-- Key formulas: SLG = (H+2B+2*3B+3*HR)/AB
-- Qualifiers: teamID IS NOT NULL, PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_slg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 21: mlb_season_batting_standard
-- Purpose: Individual season batting lines for all MLB Teams
--          hitters, aggregated across stints within a season.
-- Key formulas: AVG, OBP, SLG, OPS. PA = AB+BB+HBP+SF+SH.
-- Qualifiers: teamID IS NOT NULL, AB >= 50 in that season
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 22: mlb_season_batting_advanced
-- Purpose: Season-level advanced batting: ISO, BABIP, wOBA,
--          BB%, K% for MLB Teams hitters.
-- Key formulas: wOBA linear weights, ISO, BABIP.
-- Qualifiers: teamID IS NOT NULL, PA >= 200
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 23: mlb_single_season_hr_leaders
-- Purpose: Top HR seasons in MLB Teams franchise history.
-- Key formulas: SUM(HR) per player per year.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_hr_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 24: mlb_single_season_avg_leaders
-- Purpose: Top batting average seasons in MLB Teams history
--          (qualified: PA >= 400).
-- Key formulas: AVG = H/AB.
-- Qualifiers: teamID IS NOT NULL, PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_avg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 25: mlb_single_season_ops_leaders
-- Purpose: Top OPS seasons in MLB Teams franchise history
--          (qualified: PA >= 400).
-- Key formulas: OPS = OBP + SLG.
-- Qualifiers: teamID IS NOT NULL, PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 26: mlb_single_season_rbi_leaders
-- Purpose: Top RBI seasons in MLB Teams history.
-- Key formulas: SUM(RBI) per player per year.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_rbi_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 27: mlb_single_season_sb_leaders
-- Purpose: Top stolen base seasons in MLB Teams history.
-- Key formulas: SUM(SB) with CS and SB% = SB/(SB+CS).
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_sb_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 28: mlb_200_hit_seasons
-- Purpose: All 200+ hit seasons in MLB Teams history.
-- Key formulas: SUM(H) per season.
-- Qualifiers: teamID IS NOT NULL, H >= 200
-- ============================================================
CREATE OR REPLACE VIEW mlb_200_hit_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 29: mlb_30_hr_seasons
-- Purpose: All 30+ HR seasons by MLB Teams players.
-- Key formulas: SUM(HR) per player-season.
-- Qualifiers: teamID IS NOT NULL, HR >= 30
-- ============================================================
CREATE OR REPLACE VIEW mlb_30_hr_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 30: mlb_power_speed_seasons
-- Purpose: MLB Teams 20/20 (HR/SB) and 30/30 seasons.
-- Key formulas: SUM(HR) and SUM(SB) per player-season.
-- Qualifiers: teamID IS NOT NULL, HR >= 20 AND SB >= 20
-- ============================================================
CREATE OR REPLACE VIEW mlb_power_speed_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 31: mlb_100_rbi_seasons
-- Purpose: All 100+ RBI seasons by MLB Teams players.
-- Key formulas: SUM(RBI) per player-season.
-- Qualifiers: teamID IS NOT NULL, RBI >= 100
-- ============================================================
CREATE OR REPLACE VIEW mlb_100_rbi_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 32: mlb_dh_seasons
-- Purpose: DH-focused seasons for MLB Teams (G_dh > 50).
-- Key formulas: Standard batting stats. Uses appearances table
--   for DH game counts.
-- Qualifiers: teamID IS NOT NULL, G_dh > 50
-- ============================================================
CREATE OR REPLACE VIEW mlb_dh_seasons AS
WITH dh_games AS (
...
FROM dh_games dg
JOIN batting b ON b.playerID = dg.playerID AND b.yearID = dg.yearID AND b.teamID IS NOT NULL
JOIN people p ON p.playerID = dg.playerID
GROUP BY p.playerID, p.nameFirst, p.nameLast, dg.yearID, dg.G_dh, dg.G_all
ORDER BY OPS DESC;

-- ============================================================
-- View 33: mlb_rookie_batting
-- Purpose: Rookie season batting stats for MLB Teams players.
-- Key formulas: Standard batting stats. Rookie = first year
--   appearing in batting table.
-- Qualifiers: teamID IS NOT NULL, player's first MLB year
-- ============================================================
CREATE OR REPLACE VIEW mlb_rookie_batting AS
WITH first_year AS (
...
-- ============================================================
-- View 34: mlb_career_pitching_standard
-- Purpose: Career pitching totals for MLB Teams pitchers
--          (only MLB Teams stints) with standard metrics.
-- Key formulas: IP = IPouts/3, ERA = 9*ER/IP,
--   WHIP = (BB+H)/IP, W% = W/(W+L)
-- Qualifiers: teamID IS NOT NULL, IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE pi.teamID IS NOT NULL
GROUP BY p.playerID, p.nameFirst, p.nameLast
HAVING SUM(pi.IPouts) >= 90
ORDER BY ERA ASC;

-- ============================================================
-- View 35: mlb_career_pitching_advanced
-- Purpose: Career advanced pitching metrics for MLB Teams
--          pitchers: K/9, BB/9, K/BB, HR/9, FIP, BABIP, K%, BB%.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID IS NOT NULL, IPouts >= 300 (100+ IP)
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 36: mlb_career_pitching_counting_leaders
-- Purpose: Career pitching counting stat leaders: W, SO, SV,
--          CG, SHO, IP with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_pitching_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 37: mlb_career_starter_stats
-- Purpose: Career stats for MLB Teams starting pitchers
--          (career GS >= 50% of G, and >= 20 GS).
-- Key formulas: ERA, WHIP, K/9, W-L, FIP.
-- Qualifiers: teamID IS NOT NULL, GS >= 50% of G, GS >= 20
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_starter_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 38: mlb_career_reliever_stats
-- Purpose: Career stats for MLB Teams relievers
--          (career GS < 10% of G, G >= 50).
-- Key formulas: ERA, WHIP, K/9, SV, holds proxy, FIP.
-- Qualifiers: teamID IS NOT NULL, GS < 10% of G, G >= 50
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_reliever_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 39: mlb_career_era_leaders
-- Purpose: Best career ERA among MLB Teams pitchers (min 200 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID IS NOT NULL, IPouts >= 600
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 40: mlb_season_pitching_standard
-- Purpose: Individual season pitching lines for all MLB Teams
--          pitchers, aggregated across stints within a season.
-- Key formulas: ERA, WHIP, K/9, W-L.
-- Qualifiers: teamID IS NOT NULL, IPouts >= 30 (10+ IP)
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 41: mlb_season_pitching_advanced
-- Purpose: Season-level advanced pitching stats for MLB Teams:
--          FIP, K%, BB%, HR/9, BABIP.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID IS NOT NULL, IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 42: mlb_single_season_era_leaders
-- Purpose: Best single-season ERA in MLB Teams history (min 100 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID IS NOT NULL, IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 43: mlb_single_season_k_leaders
-- Purpose: Top single-season strikeout totals in MLB Teams history.
-- Key formulas: SUM(SO), K/9.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_k_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 44: mlb_single_season_win_leaders
-- Purpose: Top single-season win totals in MLB Teams history.
-- Key formulas: SUM(W), W-L, ERA.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_win_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 45: mlb_single_season_save_leaders
-- Purpose: Top single-season save totals in MLB Teams history.
-- Key formulas: SUM(SV), ERA, WHIP.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_save_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 46: mlb_single_season_whip_leaders
-- Purpose: Best single-season WHIP in MLB Teams history (100+ IP).
-- Key formulas: WHIP = (BB+H)/(IPouts/3)
-- Qualifiers: teamID IS NOT NULL, IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_whip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 47: mlb_innings_eater_seasons
-- Purpose: Seasons with the most innings pitched in MLB Teams
--          history.
-- Key formulas: IPouts/3 = IP.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_innings_eater_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 48: mlb_starter_vs_reliever_split
-- Purpose: MLB Teams pitchers classified as SP vs RP each season.
-- Key formulas: SP = GS >= 50% of G, RP = GS < 50%.
-- Qualifiers: teamID IS NOT NULL, IPouts >= 30
-- ============================================================
CREATE OR REPLACE VIEW mlb_starter_vs_reliever_split AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 49: mlb_complete_game_seasons
-- Purpose: All MLB Teams pitcher-seasons with complete games.
-- Key formulas: CG, SHO, ERA.
-- Qualifiers: teamID IS NOT NULL, CG >= 1
-- ============================================================
CREATE OR REPLACE VIEW mlb_complete_game_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 50: mlb_single_season_fip_leaders
-- Purpose: Best single-season FIP in MLB Teams history (100+ IP).
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID IS NOT NULL, IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW mlb_single_season_fip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 51: mlb_career_fielding_by_position
-- Purpose: Career fielding stats by position for MLB Teams
--          players with >= 100 G at that position.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID IS NOT NULL, G >= 100 at position
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_fielding_by_position AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 52: mlb_season_fielding
-- Purpose: Season fielding stats for MLB Teams players.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID IS NOT NULL, G >= 20
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 53: mlb_gold_glove_winners
-- Purpose: MLB Teams players who won Gold Glove awards.
-- Key formulas: Joins awardsplayers with batting/fielding.
-- Qualifiers: awardID = 'Gold Glove'
-- ============================================================
CREATE OR REPLACE VIEW mlb_gold_glove_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE ap.awardID = 'Gold Glove'
  AND EXISTS (
      ...
      WHERE pi.playerID = ap.playerID
        ...
        AND pi.teamID IS NOT NULL
  )
ORDER BY ap.yearID;

-- ============================================================
-- View 54: mlb_catcher_stats
-- Purpose: Career catcher fielding + batting for MLB Teams
--          catchers with >= 50 G at catcher.
-- Key formulas: CS% = CS/(CS+SB), Fld%, batting rate stats.
-- Qualifiers: teamID IS NOT NULL, G at C >= 50
-- ============================================================
CREATE OR REPLACE VIEW mlb_catcher_stats AS
WITH catcher_field AS (
    ...
    FROM fielding f
    WHERE f.POS = 'C' AND f.teamID IS NOT NULL
    GROUP BY f.playerID
    HAVING SUM(f.G) >= 50
),
catcher_bat AS (
...
-- ============================================================
-- View 55: mlb_utility_players
-- Purpose: MLB Teams players who appeared at 3+ positions in a
--          single season (versatile defenders).
-- Key formulas: COUNT(DISTINCT POS) from fielding.
-- Qualifiers: teamID IS NOT NULL, 3+ positions in a season
-- ============================================================
CREATE OR REPLACE VIEW mlb_utility_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 57: mlb_fielding_pct_leaders
-- Purpose: Best single-season fielding percentage by position
--          in MLB Teams history (min 50 G at position).
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID IS NOT NULL, G >= 50 at position
-- ============================================================
CREATE OR REPLACE VIEW mlb_fielding_pct_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 58: mlb_all_stars
-- Purpose: All MLB Teams All-Star Game selections.
-- Key formulas: Joins allstarfull with people.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_all_stars AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE asf.teamID IS NOT NULL
ORDER BY asf.yearID, p.nameLast;

-- ============================================================
-- View 59: mlb_all_star_count
-- Purpose: MLB Teams players ranked by number of All-Star
--          selections while with the team.
-- Key formulas: COUNT(DISTINCT yearID).
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_all_star_count AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 60: mlb_mvp_voting
-- Purpose: MLB Teams players who received MVP votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'MVP'
-- ============================================================
CREATE OR REPLACE VIEW mlb_mvp_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 61: mlb_cy_young_voting
-- Purpose: MLB Teams pitchers who received Cy Young votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'Cy Young'
-- ============================================================
CREATE OR REPLACE VIEW mlb_cy_young_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 62: mlb_award_winners
-- Purpose: All individual award winners while with the MLB Teams.
-- Key formulas: Joins awardsplayers to team roster.
-- ============================================================
CREATE OR REPLACE VIEW mlb_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 63: mlb_hof_players
-- Purpose: Hall of Famers who played for the MLB Teams.
-- Key formulas: Joins halloffame with appearances.
-- Qualifiers: inducted = 'Y'
-- ============================================================
CREATE OR REPLACE VIEW mlb_hof_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
JOIN appearances a ON a.playerID = h.playerID AND a.teamID IS NOT NULL
WHERE h.inducted = 'Y'
GROUP BY p.playerID, p.nameFirst, p.nameLast,
...
-- ============================================================
-- View 64: mlb_silver_slugger_winners
-- Purpose: MLB Teams Silver Slugger award winners.
-- Key formulas: Joins awardsplayers with roster.
-- Qualifiers: awardID = 'Silver Slugger'
-- ============================================================
CREATE OR REPLACE VIEW mlb_silver_slugger_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 65: mlb_rookie_of_year_voting
-- Purpose: MLB Teams players who received ROY votes.
-- Key formulas: pointsWon from awardsshareplayers.
-- Qualifiers: awardID = 'Rookie of the Year'
-- ============================================================
CREATE OR REPLACE VIEW mlb_rookie_of_year_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 66: mlb_postseason_batting
-- Purpose: Postseason batting stats for MLB Teams players.
-- Key formulas: AVG, OBP, SLG, OPS from battingpost.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 67: mlb_postseason_pitching
-- Purpose: Postseason pitching stats for MLB Teams pitchers.
-- Key formulas: ERA, WHIP, K/9 from pitchingpost.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE pp.teamID IS NOT NULL
GROUP BY p.playerID, p.nameFirst, p.nameLast, pp.yearID, pp.round
ORDER BY pp.yearID, pp.round, ERA ASC;

-- ============================================================
-- View 68: mlb_postseason_career_batting
-- Purpose: Career postseason batting totals for MLB Teams players
--          (all rounds combined, SEA only).
-- Key formulas: AVG, OBP, SLG, OPS.
-- Qualifiers: teamID IS NOT NULL, AB >= 5
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_career_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 69: mlb_postseason_career_pitching
-- Purpose: Career postseason pitching totals for MLB Teams
--          pitchers (all rounds combined, SEA only).
-- Key formulas: ERA, WHIP, K/9.
-- Qualifiers: teamID IS NOT NULL, IPouts >= 3
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_career_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 70: mlb_postseason_series
-- Purpose: MLB Teams postseason series results from seriespost.
-- Key formulas: Wins, losses, ties per series.
-- Qualifiers: teamIDwinner IS NOT NULL OR teamIDloser IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_series AS
SELECT
    sp.yearID,
    sp.round,
    CASE
        WHEN sp.teamIDwinner IS NOT NULL THEN 'Won'
        ELSE 'Lost'
    END                                                        AS result,
    CASE
        WHEN sp.teamIDwinner IS NOT NULL THEN sp.teamIDloser
    ...
    sp.ties
FROM seriespost sp
WHERE sp.teamIDwinner IS NOT NULL OR sp.teamIDloser IS NOT NULL
ORDER BY sp.yearID, sp.round;

-- ============================================================
-- View 71: mlb_salary_history
-- Purpose: Player salary history for the MLB Teams.
-- Key formulas: salary from salaries table.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_salary_history AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 72: mlb_highest_paid_players
-- Purpose: All-time highest paid MLB Teams players by total and
--          peak salary.
-- Key formulas: SUM(salary), MAX(salary).
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_highest_paid_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 75: mlb_players_by_birthplace
-- Purpose: Where MLB Teams players were born (country/state).
-- Key formulas: COUNT by birthCountry/birthState.
-- ============================================================
CREATE OR REPLACE VIEW mlb_players_by_birthplace AS
SELECT
    p.birthCountry,
    p.birthState,
...
WHERE EXISTS (
    ...
    WHERE a.playerID = p.playerID AND a.teamID IS NOT NULL
)
GROUP BY p.birthCountry, p.birthState
ORDER BY player_count DESC;

-- ============================================================
-- View 76: mlb_college_pipeline
-- Purpose: Colleges that produced the most MLB Teams players.
-- Key formulas: COUNT(DISTINCT playerID) by school.
-- ============================================================
CREATE OR REPLACE VIEW mlb_college_pipeline AS
SELECT
    sc.schoolID,
...
WHERE EXISTS (
    SELECT 1 FROM appearances a
    WHERE a.playerID = cp.playerID AND a.teamID IS NOT NULL
)
GROUP BY sc.schoolID, sc.name_full, sc.city, sc.state
ORDER BY player_count DESC;

-- ============================================================
-- View 77: mlb_international_players
-- Purpose: MLB Teams players born outside the USA.
-- Key formulas: COUNT by country, individual player list.
-- Qualifiers: Players birthCountry != 'USA'
-- ============================================================
CREATE OR REPLACE VIEW mlb_international_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 78: mlb_player_tenure
-- Purpose: Length of each player's MLB Teams tenure.
-- Key formulas: COUNT(DISTINCT yearID), span of years.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_player_tenure AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 79: mlb_debut_players
-- Purpose: Players whose MLB debut was with the MLB Teams.
-- Key formulas: Player's first year in batting/pitching matches
--   their first year with SEA.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_debut_players AS
WITH first_mlb AS (
...
-- ============================================================
-- View 80: mlb_player_ages
-- Purpose: Age distribution of MLB Teams rosters by year.
-- Key formulas: age = yearID - birthYear.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_player_ages AS
SELECT
    a.yearID,
...
-- ============================================================
-- View 83: mlb_best_seasons
-- Purpose: Best MLB Teams team seasons ranked by win percentage.
-- Key formulas: W%, run differential, Pythagorean record.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_best_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 84: mlb_worst_seasons
-- Purpose: Worst MLB Teams team seasons ranked by win percentage.
-- Key formulas: W%, run differential.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_worst_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 86: mlb_era_comparison
-- Purpose: Compare MLB Teams performance across different eras.
-- Key formulas: W%, R/G, HR/G, ERA, attendance avg by era.
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_era_comparison AS
SELECT
    CASE
...
-- ============================================================
-- View 87: mlb_players_also_played_for
-- Purpose: MLB Teams players who also played for other teams,
--          showing which other teams they played for.
-- ============================================================
CREATE OR REPLACE VIEW mlb_players_also_played_for AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
    COUNT(DISTINCT CASE WHEN a.teamID IS NOT NULL THEN a.yearID END) AS sea_seasons,
    COUNT(DISTINCT CASE WHEN a.teamID IS NOT NULL THEN a.teamID END) AS other_teams_count,
    GROUP_CONCAT(DISTINCT CASE WHEN a.teamID IS NOT NULL THEN a.teamID END
                 ORDER BY a.teamID SEPARATOR ', ')             AS other_teams
FROM appearances a
JOIN people p ON p.playerID = a.playerID
WHERE EXISTS (
    SELECT 1 FROM appearances a2
    WHERE a2.playerID = a.playerID AND a2.teamID IS NOT NULL
)
GROUP BY p.playerID, p.nameFirst, p.nameLast
HAVING COUNT(DISTINCT CASE WHEN a.teamID IS NOT NULL THEN a.teamID END) > 0
ORDER BY other_teams_count DESC;

-- ============================================================
-- View 88: mlb_career_arc_batting
-- Purpose: Age-bucket batting arcs for MLB Teams players showing
--          performance by age group (MLB Teams stints only).
-- Key formulas: AVG/OBP/SLG/OPS by age bucket.
-- Qualifiers: teamID IS NOT NULL, career PA >= 500 in MLB
-- ============================================================
CREATE OR REPLACE VIEW mlb_career_arc_batting AS
WITH player_seasons AS (
    ...
    FROM batting b
    JOIN people p ON p.playerID = b.playerID
    WHERE b.teamID IS NOT NULL AND p.birthYear IS NOT NULL
...
-- ============================================================
-- View 89: mlb_high_k_rate_pitcher_seasons
-- Purpose: MLB Teams pitcher seasons with the highest strikeout
--          rates (K%, min 50 IP).
-- Key formulas: K% = SO/BFP, K/9.
-- Qualifiers: teamID IS NOT NULL, IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW mlb_high_k_rate_pitcher_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 90: mlb_low_era_relief_seasons
-- Purpose: Best ERA seasons by MLB Teams relievers (min 30 IP,
--          GS < 3).
-- Key formulas: ERA, WHIP, K/9 for relief-only seasons.
-- Qualifiers: teamID IS NOT NULL, IPouts >= 90, GS < 3
-- ============================================================
CREATE OR REPLACE VIEW mlb_low_era_relief_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 91: mlb_season_batting_ops_plus
-- Purpose: Season OPS+ for MLB Teams hitters using dynamic
--          league averages (PA >= 300).
-- Key formulas: OPS+ = 100*(OBP/lgOBP + SLG/lgSLG - 1)
-- Qualifiers: teamID IS NOT NULL, PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_batting_ops_plus AS
WITH season_bat AS (
    SELECT
...
-- ============================================================
-- View 92: mlb_season_pitching_era_plus
-- Purpose: Season ERA+ for MLB Teams pitchers using dynamic
--          league ERA (IPouts >= 150).
-- Key formulas: ERA+ = 100 * (lgERA / playerERA)
-- Qualifiers: teamID IS NOT NULL, IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_pitching_era_plus AS
...
-- ============================================================
-- View 93: mlb_season_woba_leaders
-- Purpose: Top single-season wOBA for MLB Teams hitters.
-- Key formulas: wOBA with linear weights.
-- Qualifiers: teamID IS NOT NULL, PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW mlb_season_woba_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 94: mlb_multi_award_winners
-- Purpose: MLB Teams players who won multiple different awards
--          while with the team.
-- Key formulas: COUNT(DISTINCT awardID), list of awards.
-- Qualifiers: 2+ distinct awards
-- ============================================================
CREATE OR REPLACE VIEW mlb_multi_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 95: mlb_high_obp_seasons
-- Purpose: MLB Teams seasons with OBP >= .370 (qualified).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID IS NOT NULL, PA >= 400, OBP >= .370
-- ============================================================
CREATE OR REPLACE VIEW mlb_high_obp_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 96: mlb_salary_by_position
-- Purpose: Average salary by primary position for MLB Teams.
-- Key formulas: AVG(salary) grouped by primary position.
-- Qualifiers: teamID IS NOT NULL, salary data available
-- ============================================================
CREATE OR REPLACE VIEW mlb_salary_by_position AS
WITH sea_pos AS (
    SELECT a.playerID, a.yearID,
        CASE
...
-- ============================================================
-- View 97: mlb_postseason_fielding
-- Purpose: Postseason fielding stats for MLB Teams.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID IS NOT NULL
-- ============================================================
CREATE OR REPLACE VIEW mlb_postseason_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE fp.teamID IS NOT NULL
GROUP BY p.playerID, p.nameFirst, p.nameLast, fp.yearID, fp.round, fp.POS
ORDER BY fp.yearID, fp.round, fp.POS;

-- ============================================================
-- View 98: mlb_composite_player_value
-- Purpose: Composite player value score for MLB Teams hitters
...
-- Qualifiers: teamID IS NOT NULL, PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW mlb_composite_player_value AS
WITH career AS (
    SELECT
        b.playerID,
        SUM(b.AB) + SUM(COALESCE(b.BB, 0)) + SUM(COALESCE(b.HBP, 0))
...
-- ============================================================
-- View 99: mlb_all_time_best_by_position
-- Purpose: Best Mariner at each position by career OPS
--          (min 200 G at that position in MLB).
-- Key formulas: OPS, games at position from fielding.
-- Qualifiers: teamID IS NOT NULL, G at position >= 200
-- ============================================================
CREATE OR REPLACE VIEW mlb_all_time_best_by_position AS
WITH pos_games AS (
...
-- ============================================================
-- View 100: mlb_hall_of_fame_monitor
-- Purpose: HOF-relevant stats for MLB Teams players: career
...
--   and all-star data (MLB Teams stints only).
-- Qualifiers: PA >= 1000 OR IP >= 300 in MLB
-- ============================================================
CREATE OR REPLACE VIEW mlb_hall_of_fame_monitor AS
WITH bat_career AS (
    SELECT playerID,
    ...
    FROM batting WHERE teamID IS NOT NULL
    GROUP BY playerID
),
pitch_career AS (
    SELECT playerID,
    ...
    FROM pitching WHERE teamID IS NOT NULL
    GROUP BY playerID
),
awards AS (
    ...
    WHERE EXISTS (
        SELECT 1 FROM batting b WHERE b.playerID = ap.playerID
            AND b.yearID = ap.yearID AND b.teamID IS NOT NULL
        UNION ALL
        SELECT 1 FROM pitching pi WHERE pi.playerID = ap.playerID
            AND pi.yearID = ap.yearID AND pi.teamID IS NOT NULL
    )
    GROUP BY ap.playerID
),
allstars AS (
    SELECT playerID, COUNT(DISTINCT yearID) AS allstar_cnt
    FROM allstarfull WHERE teamID IS NOT NULL
    GROUP BY playerID
),
hof AS (
    SELECT playerID,
