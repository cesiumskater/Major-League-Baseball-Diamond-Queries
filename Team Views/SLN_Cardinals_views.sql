-- =====================================================================
-- ST. LOUIS CARDINALS - 100 DATABASE VIEWS
-- Team-specific analytics for the St. Louis Cardinals (teamID = 'SLN')
-- Franchise established: 1882
-- =====================================================================


-- ============================================================
-- View 1: cardinals_season_results
-- Purpose: Year-by-year W/L record with postseason flags and
--          run differential for every Cardinals season.
-- Key formulas: W%, run_diff = R - RA,
--   Pyth W% = R^2 / (R^2 + RA^2)
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_results AS
SELECT
    t.yearID,
    t.lgID,
    t.divID,
...
-- ============================================================
-- View 2: cardinals_decade_summary
-- Purpose: Cardinals performance aggregated by decade.
-- Key formulas: W%, total R, total RA, run differential,
--   division titles, playoff appearances per decade.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_decade_summary AS
SELECT
...
-- ============================================================
-- View 3: cardinals_all_time_roster
-- Purpose: Every player who appeared for the Cardinals with
...
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_all_time_roster AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 4: cardinals_managers_record
-- Purpose: All Cardinals managers with win/loss record and
--          seasons managed.
-- Key formulas: W%, years managed.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_managers_record AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 6: cardinals_home_parks
-- Purpose: Home stadiums/parks used by the Cardinals over time.
-- Key formulas: Games and attendance by park.
-- Qualifiers: teamkey = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_home_parks AS
SELECT
...
WHERE hg.teamkey = 'SLN'
ORDER BY hg.yearkey, pk.parkname;

-- ============================================================
-- View 7: cardinals_run_differential
...
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_run_differential AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 8: cardinals_nl_central_standings
-- Purpose: Cardinals standing within the NL Central each year,
--          compared to division rivals.
-- Key formulas: W%, team rank within division.
-- Qualifiers: NL Central teams (divID = 'C', lgID = 'NL')
-- Notes: Shows full NL Central standings for context.
-- ============================================================
CREATE OR REPLACE VIEW cardinals_nl_central_standings AS
SELECT
    t.yearID,
    t.teamID,
    t.name                                                     AS team_name,
    ...
    t.DivWin,
    t.WCWin,
    CASE WHEN t.teamID = 'SLN' THEN 'Y' ELSE 'N' END         AS is_cardinals
FROM teams t
WHERE t.lgID = 'NL'
  AND t.divID = 'C'
  AND t.yearID >= 1882
...
-- ============================================================
-- View 9: cardinals_postseason_years
-- Purpose: Summary of every Cardinals postseason appearance.
-- Key formulas: W%, postseason flags from teams table.
-- Qualifiers: teamID = 'SLN', made postseason
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_years AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 10: cardinals_franchise_milestones
...
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_franchise_milestones AS
SELECT
...
-- ============================================================
-- View 11: cardinals_career_batting_standard
-- Purpose: Career batting totals for all Cardinals hitters
--          (only counting Cardinals stints), with standard
--          rate stats AVG/OBP/SLG/OPS.
-- Key formulas: Standard rate stats.
-- Qualifiers: Cardinals stints only (teamID = 'SLN'), AB >= 100
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 12: cardinals_career_batting_advanced
-- Purpose: Career advanced batting metrics for Cardinals hitters:
--          ISO, BABIP, wOBA, BB%, K%.
-- Key formulas: ISO = SLG - AVG, BABIP = (H-HR)/(AB-SO-HR+SF),
--   wOBA with linear weights, BB% = BB/PA, K% = SO/PA
-- Qualifiers: teamID = 'SLN', PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 13: cardinals_career_batting_counting_leaders
-- Purpose: Career counting stat leaders for Cardinals:
--          HR, RBI, R, H, SB with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_batting_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 14: cardinals_career_ops_leaders
-- Purpose: Top career OPS among Cardinals hitters with minimum
--          1000 PA for the Cardinals.
-- Key formulas: OBP + SLG = OPS.
-- Qualifiers: teamID = 'SLN', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 15: cardinals_career_batting_war_approx
-- Purpose: Approximate career WAR for Cardinals position players
...
-- Qualifiers: teamID = 'SLN', career PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_batting_war_approx AS
WITH sea_batting AS (
...
-- ============================================================
-- View 16: cardinals_career_batting_by_position
-- Purpose: Aggregate batting performance by primary position
--          for Cardinals players across franchise history.
-- Key formulas: AVG, OBP, SLG, OPS by position.
-- Qualifiers: teamID = 'SLN', PA >= 200 per player
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_batting_by_position AS
WITH sea_pos AS (
    SELECT a.playerID,
        CASE
            WHEN SUM(COALESCE(a.G_c, 0)) >= GREATEST(SUM(COALESCE(a.G_1b, 0)), SUM(COALESCE(a.G_2b, 0)), SUM(COALESCE(a.G_3b, 0)), SUM(COALESCE(a.G_ss, 0)), SUM(COALESCE(a.G_lf, 0)), SUM(COALESCE(a.G_cf, 0)), SUM(COALESCE(a.G_rf, 0)), SUM(COALESCE(a.G_dh, 0))) THEN 'C'
...
-- ============================================================
-- View 17: cardinals_100_hr_club
-- Purpose: Cardinals players who hit 100+ HR in their Cardinals
--          career.
-- Key formulas: SUM(HR), career rate stats.
-- Qualifiers: teamID = 'SLN', career HR >= 100
-- ============================================================
CREATE OR REPLACE VIEW cardinals_100_hr_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 18: cardinals_1000_hit_club
-- Purpose: Cardinals players who recorded 1000+ hits for the Cardinals.
-- Key formulas: SUM(H), career rate stats.
-- Qualifiers: teamID = 'SLN', career H >= 1000
-- ============================================================
CREATE OR REPLACE VIEW cardinals_1000_hit_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 19: cardinals_career_obp_leaders
-- Purpose: Top career OBP among Cardinals hitters (min 1000 PA).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID = 'SLN', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_obp_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 20: cardinals_career_slg_leaders
-- Purpose: Top career SLG among Cardinals hitters (min 1000 PA).
-- Key formulas: SLG = (H+2B+2*3B+3*HR)/AB
-- Qualifiers: teamID = 'SLN', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_slg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 21: cardinals_season_batting_standard
-- Purpose: Individual season batting lines for all Cardinals
--          hitters, aggregated across stints within a season.
-- Key formulas: AVG, OBP, SLG, OPS. PA = AB+BB+HBP+SF+SH.
-- Qualifiers: teamID = 'SLN', AB >= 50 in that season
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 22: cardinals_season_batting_advanced
-- Purpose: Season-level advanced batting: ISO, BABIP, wOBA,
--          BB%, K% for Cardinals hitters.
-- Key formulas: wOBA linear weights, ISO, BABIP.
-- Qualifiers: teamID = 'SLN', PA >= 200
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 23: cardinals_single_season_hr_leaders
-- Purpose: Top HR seasons in Cardinals franchise history.
-- Key formulas: SUM(HR) per player per year.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_hr_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 24: cardinals_single_season_avg_leaders
-- Purpose: Top batting average seasons in Cardinals history
--          (qualified: PA >= 400).
-- Key formulas: AVG = H/AB.
-- Qualifiers: teamID = 'SLN', PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_avg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 25: cardinals_single_season_ops_leaders
-- Purpose: Top OPS seasons in Cardinals franchise history
--          (qualified: PA >= 400).
-- Key formulas: OPS = OBP + SLG.
-- Qualifiers: teamID = 'SLN', PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 26: cardinals_single_season_rbi_leaders
-- Purpose: Top RBI seasons in Cardinals history.
-- Key formulas: SUM(RBI) per player per year.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_rbi_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 27: cardinals_single_season_sb_leaders
-- Purpose: Top stolen base seasons in Cardinals history.
-- Key formulas: SUM(SB) with CS and SB% = SB/(SB+CS).
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_sb_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 28: cardinals_200_hit_seasons
-- Purpose: All 200+ hit seasons in Cardinals history.
-- Key formulas: SUM(H) per season.
-- Qualifiers: teamID = 'SLN', H >= 200
-- Notes: Ichiro's domain.
-- ============================================================
CREATE OR REPLACE VIEW cardinals_200_hit_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 29: cardinals_30_hr_seasons
-- Purpose: All 30+ HR seasons by Cardinals players.
-- Key formulas: SUM(HR) per player-season.
-- Qualifiers: teamID = 'SLN', HR >= 30
-- ============================================================
CREATE OR REPLACE VIEW cardinals_30_hr_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 30: cardinals_power_speed_seasons
-- Purpose: Cardinals 20/20 (HR/SB) and 30/30 seasons.
-- Key formulas: SUM(HR) and SUM(SB) per player-season.
-- Qualifiers: teamID = 'SLN', HR >= 20 AND SB >= 20
-- ============================================================
CREATE OR REPLACE VIEW cardinals_power_speed_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 31: cardinals_100_rbi_seasons
-- Purpose: All 100+ RBI seasons by Cardinals players.
-- Key formulas: SUM(RBI) per player-season.
-- Qualifiers: teamID = 'SLN', RBI >= 100
-- ============================================================
CREATE OR REPLACE VIEW cardinals_100_rbi_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 32: cardinals_dh_seasons
-- Purpose: DH-focused seasons for Cardinals (G_dh > 50).
-- Key formulas: Standard batting stats. Uses appearances table
--   for DH game counts.
-- Qualifiers: teamID = 'SLN', G_dh > 50
-- ============================================================
CREATE OR REPLACE VIEW cardinals_dh_seasons AS
WITH dh_games AS (
...
FROM dh_games dg
JOIN batting b ON b.playerID = dg.playerID AND b.yearID = dg.yearID AND b.teamID = 'SLN'
JOIN people p ON p.playerID = dg.playerID
GROUP BY p.playerID, p.nameFirst, p.nameLast, dg.yearID, dg.G_dh, dg.G_all
ORDER BY OPS DESC;

-- ============================================================
-- View 33: cardinals_rookie_batting
-- Purpose: Rookie season batting stats for Cardinals players.
-- Key formulas: Standard batting stats. Rookie = first year
--   appearing in batting table.
-- Qualifiers: teamID = 'SLN', player's first MLB year
-- ============================================================
CREATE OR REPLACE VIEW cardinals_rookie_batting AS
WITH first_year AS (
...
-- ============================================================
-- View 34: cardinals_career_pitching_standard
-- Purpose: Career pitching totals for Cardinals pitchers
--          (only Cardinals stints) with standard metrics.
-- Key formulas: IP = IPouts/3, ERA = 9*ER/IP,
--   WHIP = (BB+H)/IP, W% = W/(W+L)
-- Qualifiers: teamID = 'SLN', IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 35: cardinals_career_pitching_advanced
-- Purpose: Career advanced pitching metrics for Cardinals
--          pitchers: K/9, BB/9, K/BB, HR/9, FIP, BABIP, K%, BB%.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'SLN', IPouts >= 300 (100+ IP)
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 36: cardinals_career_pitching_counting_leaders
-- Purpose: Career pitching counting stat leaders: W, SO, SV,
--          CG, SHO, IP with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_pitching_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 37: cardinals_career_starter_stats
-- Purpose: Career stats for Cardinals starting pitchers
--          (career GS >= 50% of G, and >= 20 GS).
-- Key formulas: ERA, WHIP, K/9, W-L, FIP.
-- Qualifiers: teamID = 'SLN', GS >= 50% of G, GS >= 20
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_starter_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 38: cardinals_career_reliever_stats
-- Purpose: Career stats for Cardinals relievers
--          (career GS < 10% of G, G >= 50).
-- Key formulas: ERA, WHIP, K/9, SV, holds proxy, FIP.
-- Qualifiers: teamID = 'SLN', GS < 10% of G, G >= 50
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_reliever_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 39: cardinals_career_era_leaders
-- Purpose: Best career ERA among Cardinals pitchers (min 200 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID = 'SLN', IPouts >= 600
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 40: cardinals_season_pitching_standard
-- Purpose: Individual season pitching lines for all Cardinals
--          pitchers, aggregated across stints within a season.
-- Key formulas: ERA, WHIP, K/9, W-L.
-- Qualifiers: teamID = 'SLN', IPouts >= 30 (10+ IP)
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 41: cardinals_season_pitching_advanced
-- Purpose: Season-level advanced pitching stats for Cardinals:
--          FIP, K%, BB%, HR/9, BABIP.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'SLN', IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 42: cardinals_single_season_era_leaders
-- Purpose: Best single-season ERA in Cardinals history (min 100 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID = 'SLN', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 43: cardinals_single_season_k_leaders
-- Purpose: Top single-season strikeout totals in Cardinals history.
-- Key formulas: SUM(SO), K/9.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_k_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 44: cardinals_single_season_win_leaders
-- Purpose: Top single-season win totals in Cardinals history.
-- Key formulas: SUM(W), W-L, ERA.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_win_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 45: cardinals_single_season_save_leaders
-- Purpose: Top single-season save totals in Cardinals history.
-- Key formulas: SUM(SV), ERA, WHIP.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_save_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 46: cardinals_single_season_whip_leaders
-- Purpose: Best single-season WHIP in Cardinals history (100+ IP).
-- Key formulas: WHIP = (BB+H)/(IPouts/3)
-- Qualifiers: teamID = 'SLN', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_whip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 47: cardinals_innings_eater_seasons
-- Purpose: Seasons with the most innings pitched in Cardinals
--          history.
-- Key formulas: IPouts/3 = IP.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_innings_eater_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 48: cardinals_starter_vs_reliever_split
-- Purpose: Cardinals pitchers classified as SP vs RP each season.
-- Key formulas: SP = GS >= 50% of G, RP = GS < 50%.
-- Qualifiers: teamID = 'SLN', IPouts >= 30
-- ============================================================
CREATE OR REPLACE VIEW cardinals_starter_vs_reliever_split AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 49: cardinals_complete_game_seasons
-- Purpose: All Cardinals pitcher-seasons with complete games.
-- Key formulas: CG, SHO, ERA.
-- Qualifiers: teamID = 'SLN', CG >= 1
-- ============================================================
CREATE OR REPLACE VIEW cardinals_complete_game_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 50: cardinals_single_season_fip_leaders
-- Purpose: Best single-season FIP in Cardinals history (100+ IP).
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'SLN', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW cardinals_single_season_fip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 51: cardinals_career_fielding_by_position
-- Purpose: Career fielding stats by position for Cardinals
--          players with >= 100 G at that position.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'SLN', G >= 100 at position
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_fielding_by_position AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 52: cardinals_season_fielding
-- Purpose: Season fielding stats for Cardinals players.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'SLN', G >= 20
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 53: cardinals_gold_glove_winners
-- Purpose: Cardinals players who won Gold Glove awards.
-- Key formulas: Joins awardsplayers with batting/fielding.
-- Qualifiers: awardID = 'Gold Glove', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW cardinals_gold_glove_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 54: cardinals_catcher_stats
-- Purpose: Career catcher fielding + batting for Cardinals
--          catchers with >= 50 G at catcher.
-- Key formulas: CS% = CS/(CS+SB), Fld%, batting rate stats.
-- Qualifiers: teamID = 'SLN', G at C >= 50
-- ============================================================
CREATE OR REPLACE VIEW cardinals_catcher_stats AS
WITH catcher_field AS (
    ...
    FROM fielding f
    WHERE f.POS = 'C' AND f.teamID = 'SLN'
    GROUP BY f.playerID
    HAVING SUM(f.G) >= 50
),
catcher_bat AS (
...
-- ============================================================
-- View 55: cardinals_utility_players
-- Purpose: Cardinals players who appeared at 3+ positions in a
--          single season (versatile defenders).
-- Key formulas: COUNT(DISTINCT POS) from fielding.
-- Qualifiers: teamID = 'SLN', 3+ positions in a season
-- ============================================================
CREATE OR REPLACE VIEW cardinals_utility_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 57: cardinals_fielding_pct_leaders
-- Purpose: Best single-season fielding percentage by position
--          in Cardinals history (min 50 G at position).
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'SLN', G >= 50 at position
-- ============================================================
CREATE OR REPLACE VIEW cardinals_fielding_pct_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 58: cardinals_all_stars
-- Purpose: All Cardinals All-Star Game selections.
-- Key formulas: Joins allstarfull with people.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_all_stars AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE asf.teamID = 'SLN'
ORDER BY asf.yearID, p.nameLast;

-- ============================================================
-- View 59: cardinals_all_star_count
-- Purpose: Cardinals players ranked by number of All-Star
--          selections while with the team.
-- Key formulas: COUNT(DISTINCT yearID).
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_all_star_count AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 60: cardinals_mvp_voting
-- Purpose: Cardinals players who received MVP votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'MVP', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW cardinals_mvp_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 61: cardinals_cy_young_voting
-- Purpose: Cardinals pitchers who received Cy Young votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'Cy Young', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW cardinals_cy_young_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 62: cardinals_award_winners
-- Purpose: All individual award winners while with the Cardinals.
-- Key formulas: Joins awardsplayers to team roster.
-- Qualifiers: Player on SEA roster in award year
-- ============================================================
CREATE OR REPLACE VIEW cardinals_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 63: cardinals_hof_players
-- Purpose: Hall of Famers who played for the Cardinals.
-- Key formulas: Joins halloffame with appearances.
-- Qualifiers: inducted = 'Y', played for SEA
-- ============================================================
CREATE OR REPLACE VIEW cardinals_hof_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
JOIN appearances a ON a.playerID = h.playerID AND a.teamID = 'SLN'
WHERE h.inducted = 'Y'
GROUP BY p.playerID, p.nameFirst, p.nameLast,
...
-- ============================================================
-- View 64: cardinals_silver_slugger_winners
-- Purpose: Cardinals Silver Slugger award winners.
-- Key formulas: Joins awardsplayers with roster.
-- Qualifiers: awardID = 'Silver Slugger', on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW cardinals_silver_slugger_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 65: cardinals_rookie_of_year_voting
-- Purpose: Cardinals players who received ROY votes.
-- Key formulas: pointsWon from awardsshareplayers.
-- Qualifiers: awardID = 'Rookie of the Year', on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW cardinals_rookie_of_year_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 66: cardinals_postseason_batting
-- Purpose: Postseason batting stats for Cardinals players.
-- Key formulas: AVG, OBP, SLG, OPS from battingpost.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 67: cardinals_postseason_pitching
-- Purpose: Postseason pitching stats for Cardinals pitchers.
-- Key formulas: ERA, WHIP, K/9 from pitchingpost.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 68: cardinals_postseason_career_batting
-- Purpose: Career postseason batting totals for Cardinals players
--          (all rounds combined, SEA only).
-- Key formulas: AVG, OBP, SLG, OPS.
-- Qualifiers: teamID = 'SLN', AB >= 5
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_career_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 69: cardinals_postseason_career_pitching
-- Purpose: Career postseason pitching totals for Cardinals
--          pitchers (all rounds combined, SEA only).
-- Key formulas: ERA, WHIP, K/9.
-- Qualifiers: teamID = 'SLN', IPouts >= 3
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_career_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 70: cardinals_postseason_series
-- Purpose: Cardinals postseason series results from seriespost.
-- Key formulas: Wins, losses, ties per series.
-- Qualifiers: teamIDwinner = 'SLN' OR teamIDloser = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_series AS
SELECT
    sp.yearID,
    sp.round,
    CASE
        WHEN sp.teamIDwinner = 'SLN' THEN 'Won'
        ELSE 'Lost'
    END                                                        AS result,
    CASE
        WHEN sp.teamIDwinner = 'SLN' THEN sp.teamIDloser
    ...
    sp.ties
FROM seriespost sp
WHERE sp.teamIDwinner = 'SLN' OR sp.teamIDloser = 'SLN'
ORDER BY sp.yearID, sp.round;

-- ============================================================
-- View 71: cardinals_salary_history
-- Purpose: Player salary history for the Cardinals.
-- Key formulas: salary from salaries table.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_salary_history AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 72: cardinals_highest_paid_players
-- Purpose: All-time highest paid Cardinals players by total and
--          peak salary.
-- Key formulas: SUM(salary), MAX(salary).
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_highest_paid_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 75: cardinals_players_by_birthplace
-- Purpose: Where Cardinals players were born (country/state).
-- Key formulas: COUNT by birthCountry/birthState.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW cardinals_players_by_birthplace AS
SELECT
    p.birthCountry,
    p.birthState,
...
WHERE EXISTS (
    ...
    WHERE a.playerID = p.playerID AND a.teamID = 'SLN'
)
GROUP BY p.birthCountry, p.birthState
ORDER BY player_count DESC;

-- ============================================================
-- View 76: cardinals_college_pipeline
-- Purpose: Colleges that produced the most Cardinals players.
-- Key formulas: COUNT(DISTINCT playerID) by school.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW cardinals_college_pipeline AS
SELECT
    sc.schoolID,
...
WHERE EXISTS (
    SELECT 1 FROM appearances a
    WHERE a.playerID = cp.playerID AND a.teamID = 'SLN'
)
GROUP BY sc.schoolID, sc.name_full, sc.city, sc.state
ORDER BY player_count DESC;

-- ============================================================
-- View 77: cardinals_international_players
-- Purpose: Cardinals players born outside the USA.
-- Key formulas: COUNT by country, individual player list.
-- Qualifiers: Players who appeared for SEA, birthCountry != 'USA'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_international_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 78: cardinals_player_tenure
-- Purpose: Length of each player's Cardinals tenure.
-- Key formulas: COUNT(DISTINCT yearID), span of years.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_player_tenure AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 79: cardinals_debut_players
-- Purpose: Players whose MLB debut was with the Cardinals.
-- Key formulas: Player's first year in batting/pitching matches
--   their first year with SEA.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_debut_players AS
WITH first_mlb AS (
...
-- ============================================================
-- View 80: cardinals_player_ages
-- Purpose: Age distribution of Cardinals rosters by year.
-- Key formulas: age = yearID - birthYear.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_player_ages AS
SELECT
    a.yearID,
...
-- ============================================================
-- View 82: cardinals_team_pitching_by_year
-- Purpose: Team-level pitching aggregates by year from the
--          teams table.
-- Key formulas: ERA, WHIP, K/9 at team level.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_team_pitching_by_year AS
SELECT
    t.yearID,
    t.G,
    t.W,
...
-- ============================================================
-- View 83: cardinals_best_seasons
-- Purpose: Best Cardinals team seasons ranked by win percentage.
-- Key formulas: W%, run differential, Pythagorean record.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_best_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 84: cardinals_worst_seasons
-- Purpose: Worst Cardinals team seasons ranked by win percentage.
-- Key formulas: W%, run differential.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_worst_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 86: cardinals_era_comparison
-- Purpose: Compare Cardinals performance across different eras.
-- Key formulas: W%, R/G, HR/G, ERA, attendance avg by era.
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_era_comparison AS
SELECT
    CASE
...
-- ============================================================
-- View 87: cardinals_players_also_played_for
-- Purpose: Cardinals players who also played for other teams,
--          showing which other teams they played for.
-- Key formulas: GROUP_CONCAT of non-SEA teamIDs.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW cardinals_players_also_played_for AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
    COUNT(DISTINCT CASE WHEN a.teamID = 'SLN' THEN a.yearID END) AS sea_seasons,
    COUNT(DISTINCT CASE WHEN a.teamID != 'SLN' THEN a.teamID END) AS other_teams_count,
    GROUP_CONCAT(DISTINCT CASE WHEN a.teamID != 'SLN' THEN a.teamID END
                 ORDER BY a.teamID SEPARATOR ', ')             AS other_teams
FROM appearances a
JOIN people p ON p.playerID = a.playerID
WHERE EXISTS (
    SELECT 1 FROM appearances a2
    WHERE a2.playerID = a.playerID AND a2.teamID = 'SLN'
)
GROUP BY p.playerID, p.nameFirst, p.nameLast
HAVING COUNT(DISTINCT CASE WHEN a.teamID != 'SLN' THEN a.teamID END) > 0
ORDER BY other_teams_count DESC;

-- ============================================================
-- View 88: cardinals_career_arc_batting
-- Purpose: Age-bucket batting arcs for Cardinals players showing
--          performance by age group (Cardinals stints only).
-- Key formulas: AVG/OBP/SLG/OPS by age bucket.
-- Qualifiers: teamID = 'SLN', career PA >= 500 for the Cardinals
-- ============================================================
CREATE OR REPLACE VIEW cardinals_career_arc_batting AS
WITH player_seasons AS (
    ...
    FROM batting b
    JOIN people p ON p.playerID = b.playerID
    WHERE b.teamID = 'SLN' AND p.birthYear IS NOT NULL
...
-- ============================================================
-- View 89: cardinals_high_k_rate_pitcher_seasons
-- Purpose: Cardinals pitcher seasons with the highest strikeout
--          rates (K%, min 50 IP).
-- Key formulas: K% = SO/BFP, K/9.
-- Qualifiers: teamID = 'SLN', IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW cardinals_high_k_rate_pitcher_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 90: cardinals_low_era_relief_seasons
-- Purpose: Best ERA seasons by Cardinals relievers (min 30 IP,
--          GS < 3).
-- Key formulas: ERA, WHIP, K/9 for relief-only seasons.
-- Qualifiers: teamID = 'SLN', IPouts >= 90, GS < 3
-- ============================================================
CREATE OR REPLACE VIEW cardinals_low_era_relief_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 91: cardinals_season_batting_ops_plus
-- Purpose: Season OPS+ for Cardinals hitters using dynamic
--          league averages (PA >= 300).
-- Key formulas: OPS+ = 100*(OBP/lgOBP + SLG/lgSLG - 1)
-- Qualifiers: teamID = 'SLN', PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_batting_ops_plus AS
WITH season_bat AS (
    SELECT
...
-- ============================================================
-- View 92: cardinals_season_pitching_era_plus
-- Purpose: Season ERA+ for Cardinals pitchers using dynamic
--          league ERA (IPouts >= 150).
-- Key formulas: ERA+ = 100 * (lgERA / playerERA)
-- Qualifiers: teamID = 'SLN', IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_pitching_era_plus AS
...
-- ============================================================
-- View 93: cardinals_season_woba_leaders
-- Purpose: Top single-season wOBA for Cardinals hitters.
-- Key formulas: wOBA with linear weights.
-- Qualifiers: teamID = 'SLN', PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW cardinals_season_woba_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 94: cardinals_multi_award_winners
-- Purpose: Cardinals players who won multiple different awards
--          while with the team.
-- Key formulas: COUNT(DISTINCT awardID), list of awards.
-- Qualifiers: Player on SEA roster, 2+ distinct awards
-- ============================================================
CREATE OR REPLACE VIEW cardinals_multi_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 95: cardinals_high_obp_seasons
-- Purpose: Cardinals seasons with OBP >= .370 (qualified).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID = 'SLN', PA >= 400, OBP >= .370
-- ============================================================
CREATE OR REPLACE VIEW cardinals_high_obp_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 96: cardinals_salary_by_position
-- Purpose: Average salary by primary position for Cardinals.
-- Key formulas: AVG(salary) grouped by primary position.
-- Qualifiers: teamID = 'SLN', salary data available
-- ============================================================
CREATE OR REPLACE VIEW cardinals_salary_by_position AS
WITH sea_pos AS (
    SELECT a.playerID, a.yearID,
        CASE
...
-- ============================================================
-- View 97: cardinals_postseason_fielding
-- Purpose: Postseason fielding stats for Cardinals.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'SLN'
-- ============================================================
CREATE OR REPLACE VIEW cardinals_postseason_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 98: cardinals_composite_player_value
-- Purpose: Composite player value score for Cardinals hitters
...
-- Qualifiers: teamID = 'SLN', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW cardinals_composite_player_value AS
WITH career AS (
    SELECT
        b.playerID,
        SUM(b.AB) + SUM(COALESCE(b.BB, 0)) + SUM(COALESCE(b.HBP, 0))
...
-- ============================================================
-- View 99: cardinals_all_time_best_by_position
-- Purpose: Best Cardinals at each position by career OPS
--          (min 200 G at that position for the Cardinals).
-- Key formulas: OPS, games at position from fielding.
-- Qualifiers: teamID = 'SLN', G at position >= 200
-- ============================================================
CREATE OR REPLACE VIEW cardinals_all_time_best_by_position AS
WITH pos_games AS (
...
-- ============================================================
-- View 100: cardinals_hall_of_fame_monitor
-- Purpose: HOF-relevant stats for Cardinals players: career
...
--   and all-star data (Cardinals stints only).
-- Qualifiers: PA >= 1000 OR IP >= 300 for the Cardinals
-- ============================================================
CREATE OR REPLACE VIEW cardinals_hall_of_fame_monitor AS
WITH bat_career AS (
    SELECT playerID,
...
pitch_career AS (
    ...
    FROM pitching WHERE teamID = 'SLN'
    GROUP BY playerID
),
awards AS (
    ...
    WHERE EXISTS (
        SELECT 1 FROM batting b WHERE b.playerID = ap.playerID
            AND b.yearID = ap.yearID AND b.teamID = 'SLN'
        UNION ALL
        SELECT 1 FROM pitching pi WHERE pi.playerID = ap.playerID
            AND pi.yearID = ap.yearID AND pi.teamID = 'SLN'
    )
    GROUP BY ap.playerID
),
allstars AS (
    SELECT playerID, COUNT(DISTINCT yearID) AS allstar_cnt
    FROM allstarfull WHERE teamID = 'SLN'
    GROUP BY playerID
),
hof AS (
    SELECT playerID,
