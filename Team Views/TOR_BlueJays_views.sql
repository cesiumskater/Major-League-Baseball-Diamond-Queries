-- =====================================================================
-- TORONTO BLUE JAYS - 100 DATABASE VIEWS
-- Team-specific analytics for the Toronto Blue Jays (teamID = 'TOR')
-- Franchise established: 1977
-- =====================================================================


-- ============================================================
-- View 1: blue_jays_season_results
-- Purpose: Year-by-year W/L record with postseason flags and
--          run differential for every Blue Jays season.
-- Key formulas: W%, run_diff = R - RA,
--   Pyth W% = R^2 / (R^2 + RA^2)
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_results AS
SELECT
    t.yearID,
    t.lgID,
    t.divID,
...
-- ============================================================
-- View 2: blue_jays_decade_summary
-- Purpose: Blue Jays performance aggregated by decade.
-- Key formulas: W%, total R, total RA, run differential,
--   division titles, playoff appearances per decade.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_decade_summary AS
SELECT
...
-- ============================================================
-- View 3: blue_jays_all_time_roster
-- Purpose: Every player who appeared for the Blue Jays with
...
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_all_time_roster AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 4: blue_jays_managers_record
-- Purpose: All Blue Jays managers with win/loss record and
--          seasons managed.
-- Key formulas: W%, years managed.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_managers_record AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 6: blue_jays_home_parks
-- Purpose: Home stadiums/parks used by the Blue Jays over time.
-- Key formulas: Games and attendance by park.
-- Qualifiers: teamkey = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_home_parks AS
SELECT
...
WHERE hg.teamkey = 'TOR'
ORDER BY hg.yearkey, pk.parkname;

-- ============================================================
-- View 7: blue_jays_run_differential
...
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_run_differential AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 8: blue_jays_al_east_standings
-- Purpose: Blue Jays standing within the AL East each year,
--          compared to division rivals.
-- Key formulas: W%, team rank within division.
-- Qualifiers: AL East teams (divID = 'E', lgID = 'AL')
-- Notes: Shows full AL East standings for context.
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_al_east_standings AS
SELECT
    t.yearID,
    t.teamID,
    t.name                                                     AS team_name,
    ...
    t.DivWin,
    t.WCWin,
    CASE WHEN t.teamID = 'TOR' THEN 'Y' ELSE 'N' END         AS is_blue_jays
FROM teams t
WHERE t.lgID = 'AL'
  AND t.divID = 'E'
  AND t.yearID >= 1977
...
-- ============================================================
-- View 9: blue_jays_postseason_years
-- Purpose: Summary of every Blue Jays postseason appearance.
-- Key formulas: W%, postseason flags from teams table.
-- Qualifiers: teamID = 'TOR', made postseason
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_years AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 10: blue_jays_franchise_milestones
...
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_franchise_milestones AS
SELECT
...
-- ============================================================
-- View 11: blue_jays_career_batting_standard
-- Purpose: Career batting totals for all Blue Jays hitters
--          (only counting Blue Jays stints), with standard
--          rate stats AVG/OBP/SLG/OPS.
-- Key formulas: Standard rate stats.
-- Qualifiers: Blue Jays stints only (teamID = 'TOR'), AB >= 100
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 12: blue_jays_career_batting_advanced
-- Purpose: Career advanced batting metrics for Blue Jays hitters:
--          ISO, BABIP, wOBA, BB%, K%.
-- Key formulas: ISO = SLG - AVG, BABIP = (H-HR)/(AB-SO-HR+SF),
--   wOBA with linear weights, BB% = BB/PA, K% = SO/PA
-- Qualifiers: teamID = 'TOR', PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 13: blue_jays_career_batting_counting_leaders
-- Purpose: Career counting stat leaders for Blue Jays:
--          HR, RBI, R, H, SB with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_batting_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 14: blue_jays_career_ops_leaders
-- Purpose: Top career OPS among Blue Jays hitters with minimum
--          1000 PA for the Blue Jays.
-- Key formulas: OBP + SLG = OPS.
-- Qualifiers: teamID = 'TOR', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 15: blue_jays_career_batting_war_approx
-- Purpose: Approximate career WAR for Blue Jays position players
...
-- Qualifiers: teamID = 'TOR', career PA >= 500
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_batting_war_approx AS
WITH sea_batting AS (
...
-- ============================================================
-- View 16: blue_jays_career_batting_by_position
-- Purpose: Aggregate batting performance by primary position
--          for Blue Jays players across franchise history.
-- Key formulas: AVG, OBP, SLG, OPS by position.
-- Qualifiers: teamID = 'TOR', PA >= 200 per player
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_batting_by_position AS
WITH sea_pos AS (
    SELECT a.playerID,
        CASE
            WHEN SUM(COALESCE(a.G_c, 0)) >= GREATEST(SUM(COALESCE(a.G_1b, 0)), SUM(COALESCE(a.G_2b, 0)), SUM(COALESCE(a.G_3b, 0)), SUM(COALESCE(a.G_ss, 0)), SUM(COALESCE(a.G_lf, 0)), SUM(COALESCE(a.G_cf, 0)), SUM(COALESCE(a.G_rf, 0)), SUM(COALESCE(a.G_dh, 0))) THEN 'C'
...
-- ============================================================
-- View 17: blue_jays_100_hr_club
-- Purpose: Blue Jays players who hit 100+ HR in their Blue Jays
--          career.
-- Key formulas: SUM(HR), career rate stats.
-- Qualifiers: teamID = 'TOR', career HR >= 100
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_100_hr_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 18: blue_jays_1000_hit_club
-- Purpose: Blue Jays players who recorded 1000+ hits for the Blue Jays.
-- Key formulas: SUM(H), career rate stats.
-- Qualifiers: teamID = 'TOR', career H >= 1000
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_1000_hit_club AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 19: blue_jays_career_obp_leaders
-- Purpose: Top career OBP among Blue Jays hitters (min 1000 PA).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID = 'TOR', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_obp_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 20: blue_jays_career_slg_leaders
-- Purpose: Top career SLG among Blue Jays hitters (min 1000 PA).
-- Key formulas: SLG = (H+2B+2*3B+3*HR)/AB
-- Qualifiers: teamID = 'TOR', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_slg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 21: blue_jays_season_batting_standard
-- Purpose: Individual season batting lines for all Blue Jays
--          hitters, aggregated across stints within a season.
-- Key formulas: AVG, OBP, SLG, OPS. PA = AB+BB+HBP+SF+SH.
-- Qualifiers: teamID = 'TOR', AB >= 50 in that season
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_batting_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 22: blue_jays_season_batting_advanced
-- Purpose: Season-level advanced batting: ISO, BABIP, wOBA,
--          BB%, K% for Blue Jays hitters.
-- Key formulas: wOBA linear weights, ISO, BABIP.
-- Qualifiers: teamID = 'TOR', PA >= 200
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_batting_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 23: blue_jays_single_season_hr_leaders
-- Purpose: Top HR seasons in Blue Jays franchise history.
-- Key formulas: SUM(HR) per player per year.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_hr_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 24: blue_jays_single_season_avg_leaders
-- Purpose: Top batting average seasons in Blue Jays history
--          (qualified: PA >= 400).
-- Key formulas: AVG = H/AB.
-- Qualifiers: teamID = 'TOR', PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_avg_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 25: blue_jays_single_season_ops_leaders
-- Purpose: Top OPS seasons in Blue Jays franchise history
--          (qualified: PA >= 400).
-- Key formulas: OPS = OBP + SLG.
-- Qualifiers: teamID = 'TOR', PA >= 400
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_ops_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 26: blue_jays_single_season_rbi_leaders
-- Purpose: Top RBI seasons in Blue Jays history.
-- Key formulas: SUM(RBI) per player per year.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_rbi_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 27: blue_jays_single_season_sb_leaders
-- Purpose: Top stolen base seasons in Blue Jays history.
-- Key formulas: SUM(SB) with CS and SB% = SB/(SB+CS).
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_sb_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 28: blue_jays_200_hit_seasons
-- Purpose: All 200+ hit seasons in Blue Jays history.
-- Key formulas: SUM(H) per season.
-- Qualifiers: teamID = 'TOR', H >= 200
-- Notes: Ichiro's domain.
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_200_hit_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 29: blue_jays_30_hr_seasons
-- Purpose: All 30+ HR seasons by Blue Jays players.
-- Key formulas: SUM(HR) per player-season.
-- Qualifiers: teamID = 'TOR', HR >= 30
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_30_hr_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 30: blue_jays_power_speed_seasons
-- Purpose: Blue Jays 20/20 (HR/SB) and 30/30 seasons.
-- Key formulas: SUM(HR) and SUM(SB) per player-season.
-- Qualifiers: teamID = 'TOR', HR >= 20 AND SB >= 20
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_power_speed_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 31: blue_jays_100_rbi_seasons
-- Purpose: All 100+ RBI seasons by Blue Jays players.
-- Key formulas: SUM(RBI) per player-season.
-- Qualifiers: teamID = 'TOR', RBI >= 100
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_100_rbi_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 32: blue_jays_dh_seasons
-- Purpose: DH-focused seasons for Blue Jays (G_dh > 50).
-- Key formulas: Standard batting stats. Uses appearances table
--   for DH game counts.
-- Qualifiers: teamID = 'TOR', G_dh > 50
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_dh_seasons AS
WITH dh_games AS (
...
FROM dh_games dg
JOIN batting b ON b.playerID = dg.playerID AND b.yearID = dg.yearID AND b.teamID = 'TOR'
JOIN people p ON p.playerID = dg.playerID
GROUP BY p.playerID, p.nameFirst, p.nameLast, dg.yearID, dg.G_dh, dg.G_all
ORDER BY OPS DESC;

-- ============================================================
-- View 33: blue_jays_rookie_batting
-- Purpose: Rookie season batting stats for Blue Jays players.
-- Key formulas: Standard batting stats. Rookie = first year
--   appearing in batting table.
-- Qualifiers: teamID = 'TOR', player's first MLB year
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_rookie_batting AS
WITH first_year AS (
...
-- ============================================================
-- View 34: blue_jays_career_pitching_standard
-- Purpose: Career pitching totals for Blue Jays pitchers
--          (only Blue Jays stints) with standard metrics.
-- Key formulas: IP = IPouts/3, ERA = 9*ER/IP,
--   WHIP = (BB+H)/IP, W% = W/(W+L)
-- Qualifiers: teamID = 'TOR', IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 35: blue_jays_career_pitching_advanced
-- Purpose: Career advanced pitching metrics for Blue Jays
--          pitchers: K/9, BB/9, K/BB, HR/9, FIP, BABIP, K%, BB%.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'TOR', IPouts >= 300 (100+ IP)
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 36: blue_jays_career_pitching_counting_leaders
-- Purpose: Career pitching counting stat leaders: W, SO, SV,
--          CG, SHO, IP with DENSE_RANK.
-- Key formulas: Simple SUMs with ranking.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_pitching_counting_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 37: blue_jays_career_starter_stats
-- Purpose: Career stats for Blue Jays starting pitchers
--          (career GS >= 50% of G, and >= 20 GS).
-- Key formulas: ERA, WHIP, K/9, W-L, FIP.
-- Qualifiers: teamID = 'TOR', GS >= 50% of G, GS >= 20
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_starter_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 38: blue_jays_career_reliever_stats
-- Purpose: Career stats for Blue Jays relievers
--          (career GS < 10% of G, G >= 50).
-- Key formulas: ERA, WHIP, K/9, SV, holds proxy, FIP.
-- Qualifiers: teamID = 'TOR', GS < 10% of G, G >= 50
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_reliever_stats AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 39: blue_jays_career_era_leaders
-- Purpose: Best career ERA among Blue Jays pitchers (min 200 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID = 'TOR', IPouts >= 600
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 40: blue_jays_season_pitching_standard
-- Purpose: Individual season pitching lines for all Blue Jays
--          pitchers, aggregated across stints within a season.
-- Key formulas: ERA, WHIP, K/9, W-L.
-- Qualifiers: teamID = 'TOR', IPouts >= 30 (10+ IP)
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_pitching_standard AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 41: blue_jays_season_pitching_advanced
-- Purpose: Season-level advanced pitching stats for Blue Jays:
--          FIP, K%, BB%, HR/9, BABIP.
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'TOR', IPouts >= 90 (30+ IP)
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_pitching_advanced AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 42: blue_jays_single_season_era_leaders
-- Purpose: Best single-season ERA in Blue Jays history (min 100 IP).
-- Key formulas: ERA = 9*ER/(IPouts/3)
-- Qualifiers: teamID = 'TOR', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_era_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 43: blue_jays_single_season_k_leaders
-- Purpose: Top single-season strikeout totals in Blue Jays history.
-- Key formulas: SUM(SO), K/9.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_k_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 44: blue_jays_single_season_win_leaders
-- Purpose: Top single-season win totals in Blue Jays history.
-- Key formulas: SUM(W), W-L, ERA.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_win_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 45: blue_jays_single_season_save_leaders
-- Purpose: Top single-season save totals in Blue Jays history.
-- Key formulas: SUM(SV), ERA, WHIP.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_save_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 46: blue_jays_single_season_whip_leaders
-- Purpose: Best single-season WHIP in Blue Jays history (100+ IP).
-- Key formulas: WHIP = (BB+H)/(IPouts/3)
-- Qualifiers: teamID = 'TOR', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_whip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 47: blue_jays_innings_eater_seasons
-- Purpose: Seasons with the most innings pitched in Blue Jays
--          history.
-- Key formulas: IPouts/3 = IP.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_innings_eater_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 48: blue_jays_starter_vs_reliever_split
-- Purpose: Blue Jays pitchers classified as SP vs RP each season.
-- Key formulas: SP = GS >= 50% of G, RP = GS < 50%.
-- Qualifiers: teamID = 'TOR', IPouts >= 30
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_starter_vs_reliever_split AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 49: blue_jays_complete_game_seasons
-- Purpose: All Blue Jays pitcher-seasons with complete games.
-- Key formulas: CG, SHO, ERA.
-- Qualifiers: teamID = 'TOR', CG >= 1
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_complete_game_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 50: blue_jays_single_season_fip_leaders
-- Purpose: Best single-season FIP in Blue Jays history (100+ IP).
-- Key formulas: FIP = (13*HR+3*(BB+HBP)-2*SO)/IP + 3.10
-- Qualifiers: teamID = 'TOR', IPouts >= 300
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_single_season_fip_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 51: blue_jays_career_fielding_by_position
-- Purpose: Career fielding stats by position for Blue Jays
--          players with >= 100 G at that position.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'TOR', G >= 100 at position
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_fielding_by_position AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 52: blue_jays_season_fielding
-- Purpose: Season fielding stats for Blue Jays players.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'TOR', G >= 20
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 53: blue_jays_gold_glove_winners
-- Purpose: Blue Jays players who won Gold Glove awards.
-- Key formulas: Joins awardsplayers with batting/fielding.
-- Qualifiers: awardID = 'Gold Glove', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_gold_glove_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 54: blue_jays_catcher_stats
-- Purpose: Career catcher fielding + batting for Blue Jays
--          catchers with >= 50 G at catcher.
-- Key formulas: CS% = CS/(CS+SB), Fld%, batting rate stats.
-- Qualifiers: teamID = 'TOR', G at C >= 50
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_catcher_stats AS
WITH catcher_field AS (
    ...
    FROM fielding f
    WHERE f.POS = 'C' AND f.teamID = 'TOR'
    GROUP BY f.playerID
    HAVING SUM(f.G) >= 50
),
catcher_bat AS (
...
-- ============================================================
-- View 55: blue_jays_utility_players
-- Purpose: Blue Jays players who appeared at 3+ positions in a
--          single season (versatile defenders).
-- Key formulas: COUNT(DISTINCT POS) from fielding.
-- Qualifiers: teamID = 'TOR', 3+ positions in a season
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_utility_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 57: blue_jays_fielding_pct_leaders
-- Purpose: Best single-season fielding percentage by position
--          in Blue Jays history (min 50 G at position).
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'TOR', G >= 50 at position
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_fielding_pct_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 58: blue_jays_all_stars
-- Purpose: All Blue Jays All-Star Game selections.
-- Key formulas: Joins allstarfull with people.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_all_stars AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
WHERE asf.teamID = 'TOR'
ORDER BY asf.yearID, p.nameLast;

-- ============================================================
-- View 59: blue_jays_all_star_count
-- Purpose: Blue Jays players ranked by number of All-Star
--          selections while with the team.
-- Key formulas: COUNT(DISTINCT yearID).
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_all_star_count AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 60: blue_jays_mvp_voting
-- Purpose: Blue Jays players who received MVP votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'MVP', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_mvp_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 61: blue_jays_cy_young_voting
-- Purpose: Blue Jays pitchers who received Cy Young votes.
-- Key formulas: pointsWon, votesFirst from awardsshareplayers.
-- Qualifiers: awardID = 'Cy Young', player on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_cy_young_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 62: blue_jays_award_winners
-- Purpose: All individual award winners while with the Blue Jays.
-- Key formulas: Joins awardsplayers to team roster.
-- Qualifiers: Player on SEA roster in award year
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 63: blue_jays_hof_players
-- Purpose: Hall of Famers who played for the Blue Jays.
-- Key formulas: Joins halloffame with appearances.
-- Qualifiers: inducted = 'Y', played for SEA
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_hof_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
JOIN appearances a ON a.playerID = h.playerID AND a.teamID = 'TOR'
WHERE h.inducted = 'Y'
GROUP BY p.playerID, p.nameFirst, p.nameLast,
...
-- ============================================================
-- View 64: blue_jays_silver_slugger_winners
-- Purpose: Blue Jays Silver Slugger award winners.
-- Key formulas: Joins awardsplayers with roster.
-- Qualifiers: awardID = 'Silver Slugger', on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_silver_slugger_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 65: blue_jays_rookie_of_year_voting
-- Purpose: Blue Jays players who received ROY votes.
-- Key formulas: pointsWon from awardsshareplayers.
-- Qualifiers: awardID = 'Rookie of the Year', on SEA roster
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_rookie_of_year_voting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 66: blue_jays_postseason_batting
-- Purpose: Postseason batting stats for Blue Jays players.
-- Key formulas: AVG, OBP, SLG, OPS from battingpost.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 67: blue_jays_postseason_pitching
-- Purpose: Postseason pitching stats for Blue Jays pitchers.
-- Key formulas: ERA, WHIP, K/9 from pitchingpost.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 68: blue_jays_postseason_career_batting
-- Purpose: Career postseason batting totals for Blue Jays players
--          (all rounds combined, SEA only).
-- Key formulas: AVG, OBP, SLG, OPS.
-- Qualifiers: teamID = 'TOR', AB >= 5
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_career_batting AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 69: blue_jays_postseason_career_pitching
-- Purpose: Career postseason pitching totals for Blue Jays
--          pitchers (all rounds combined, SEA only).
-- Key formulas: ERA, WHIP, K/9.
-- Qualifiers: teamID = 'TOR', IPouts >= 3
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_career_pitching AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 70: blue_jays_postseason_series
-- Purpose: Blue Jays postseason series results from seriespost.
-- Key formulas: Wins, losses, ties per series.
-- Qualifiers: teamIDwinner = 'TOR' OR teamIDloser = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_series AS
SELECT
    sp.yearID,
    sp.round,
    CASE
        WHEN sp.teamIDwinner = 'TOR' THEN 'Won'
        ELSE 'Lost'
    END                                                        AS result,
    CASE
        WHEN sp.teamIDwinner = 'TOR' THEN sp.teamIDloser
    ...
    sp.ties
FROM seriespost sp
WHERE sp.teamIDwinner = 'TOR' OR sp.teamIDloser = 'TOR'
ORDER BY sp.yearID, sp.round;

-- ============================================================
-- View 71: blue_jays_salary_history
-- Purpose: Player salary history for the Blue Jays.
-- Key formulas: salary from salaries table.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_salary_history AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 72: blue_jays_highest_paid_players
-- Purpose: All-time highest paid Blue Jays players by total and
--          peak salary.
-- Key formulas: SUM(salary), MAX(salary).
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_highest_paid_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 75: blue_jays_players_by_birthplace
-- Purpose: Where Blue Jays players were born (country/state).
-- Key formulas: COUNT by birthCountry/birthState.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_players_by_birthplace AS
SELECT
    p.birthCountry,
    p.birthState,
...
WHERE EXISTS (
    ...
    WHERE a.playerID = p.playerID AND a.teamID = 'TOR'
)
GROUP BY p.birthCountry, p.birthState
ORDER BY player_count DESC;

-- ============================================================
-- View 76: blue_jays_college_pipeline
-- Purpose: Colleges that produced the most Blue Jays players.
-- Key formulas: COUNT(DISTINCT playerID) by school.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_college_pipeline AS
SELECT
    sc.schoolID,
...
WHERE EXISTS (
    SELECT 1 FROM appearances a
    WHERE a.playerID = cp.playerID AND a.teamID = 'TOR'
)
GROUP BY sc.schoolID, sc.name_full, sc.city, sc.state
ORDER BY player_count DESC;

-- ============================================================
-- View 77: blue_jays_international_players
-- Purpose: Blue Jays players born outside the USA.
-- Key formulas: COUNT by country, individual player list.
-- Qualifiers: Players who appeared for SEA, birthCountry != 'USA'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_international_players AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 78: blue_jays_player_tenure
-- Purpose: Length of each player's Blue Jays tenure.
-- Key formulas: COUNT(DISTINCT yearID), span of years.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_player_tenure AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 79: blue_jays_debut_players
-- Purpose: Players whose MLB debut was with the Blue Jays.
-- Key formulas: Player's first year in batting/pitching matches
--   their first year with SEA.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_debut_players AS
WITH first_mlb AS (
...
-- ============================================================
-- View 80: blue_jays_player_ages
-- Purpose: Age distribution of Blue Jays rosters by year.
-- Key formulas: age = yearID - birthYear.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_player_ages AS
SELECT
    a.yearID,
...
-- ============================================================
-- View 82: blue_jays_team_pitching_by_year
-- Purpose: Team-level pitching aggregates by year from the
--          teams table.
-- Key formulas: ERA, WHIP, K/9 at team level.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_team_pitching_by_year AS
SELECT
    t.yearID,
    t.G,
    t.W,
...
-- ============================================================
-- View 83: blue_jays_best_seasons
-- Purpose: Best Blue Jays team seasons ranked by win percentage.
-- Key formulas: W%, run differential, Pythagorean record.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_best_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 84: blue_jays_worst_seasons
-- Purpose: Worst Blue Jays team seasons ranked by win percentage.
-- Key formulas: W%, run differential.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_worst_seasons AS
SELECT
    t.yearID,
    t.W,
    t.L,
...
-- ============================================================
-- View 86: blue_jays_era_comparison
-- Purpose: Compare Blue Jays performance across different eras.
-- Key formulas: W%, R/G, HR/G, ERA, attendance avg by era.
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_era_comparison AS
SELECT
    CASE
...
-- ============================================================
-- View 87: blue_jays_players_also_played_for
-- Purpose: Blue Jays players who also played for other teams,
--          showing which other teams they played for.
-- Key formulas: GROUP_CONCAT of non-SEA teamIDs.
-- Qualifiers: Players who appeared for SEA
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_players_also_played_for AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
    COUNT(DISTINCT CASE WHEN a.teamID = 'TOR' THEN a.yearID END) AS sea_seasons,
    COUNT(DISTINCT CASE WHEN a.teamID != 'TOR' THEN a.teamID END) AS other_teams_count,
    GROUP_CONCAT(DISTINCT CASE WHEN a.teamID != 'TOR' THEN a.teamID END
                 ORDER BY a.teamID SEPARATOR ', ')             AS other_teams
FROM appearances a
JOIN people p ON p.playerID = a.playerID
WHERE EXISTS (
    SELECT 1 FROM appearances a2
    WHERE a2.playerID = a.playerID AND a2.teamID = 'TOR'
)
GROUP BY p.playerID, p.nameFirst, p.nameLast
HAVING COUNT(DISTINCT CASE WHEN a.teamID != 'TOR' THEN a.teamID END) > 0
ORDER BY other_teams_count DESC;

-- ============================================================
-- View 88: blue_jays_career_arc_batting
-- Purpose: Age-bucket batting arcs for Blue Jays players showing
--          performance by age group (Blue Jays stints only).
-- Key formulas: AVG/OBP/SLG/OPS by age bucket.
-- Qualifiers: teamID = 'TOR', career PA >= 500 for the Blue Jays
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_career_arc_batting AS
WITH player_seasons AS (
    ...
    FROM batting b
    JOIN people p ON p.playerID = b.playerID
    WHERE b.teamID = 'TOR' AND p.birthYear IS NOT NULL
...
-- ============================================================
-- View 89: blue_jays_high_k_rate_pitcher_seasons
-- Purpose: Blue Jays pitcher seasons with the highest strikeout
--          rates (K%, min 50 IP).
-- Key formulas: K% = SO/BFP, K/9.
-- Qualifiers: teamID = 'TOR', IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_high_k_rate_pitcher_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 90: blue_jays_low_era_relief_seasons
-- Purpose: Best ERA seasons by Blue Jays relievers (min 30 IP,
--          GS < 3).
-- Key formulas: ERA, WHIP, K/9 for relief-only seasons.
-- Qualifiers: teamID = 'TOR', IPouts >= 90, GS < 3
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_low_era_relief_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 91: blue_jays_season_batting_ops_plus
-- Purpose: Season OPS+ for Blue Jays hitters using dynamic
--          league averages (PA >= 300).
-- Key formulas: OPS+ = 100*(OBP/lgOBP + SLG/lgSLG - 1)
-- Qualifiers: teamID = 'TOR', PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_batting_ops_plus AS
WITH season_bat AS (
    SELECT
...
-- ============================================================
-- View 92: blue_jays_season_pitching_era_plus
-- Purpose: Season ERA+ for Blue Jays pitchers using dynamic
--          league ERA (IPouts >= 150).
-- Key formulas: ERA+ = 100 * (lgERA / playerERA)
-- Qualifiers: teamID = 'TOR', IPouts >= 150
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_pitching_era_plus AS
...
-- ============================================================
-- View 93: blue_jays_season_woba_leaders
-- Purpose: Top single-season wOBA for Blue Jays hitters.
-- Key formulas: wOBA with linear weights.
-- Qualifiers: teamID = 'TOR', PA >= 300
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_season_woba_leaders AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 94: blue_jays_multi_award_winners
-- Purpose: Blue Jays players who won multiple different awards
--          while with the team.
-- Key formulas: COUNT(DISTINCT awardID), list of awards.
-- Qualifiers: Player on SEA roster, 2+ distinct awards
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_multi_award_winners AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 95: blue_jays_high_obp_seasons
-- Purpose: Blue Jays seasons with OBP >= .370 (qualified).
-- Key formulas: OBP = (H+BB+HBP)/(AB+BB+HBP+SF)
-- Qualifiers: teamID = 'TOR', PA >= 400, OBP >= .370
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_high_obp_seasons AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 96: blue_jays_salary_by_position
-- Purpose: Average salary by primary position for Blue Jays.
-- Key formulas: AVG(salary) grouped by primary position.
-- Qualifiers: teamID = 'TOR', salary data available
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_salary_by_position AS
WITH sea_pos AS (
    SELECT a.playerID, a.yearID,
        CASE
...
-- ============================================================
-- View 97: blue_jays_postseason_fielding
-- Purpose: Postseason fielding stats for Blue Jays.
-- Key formulas: Fld% = (PO+A)/(PO+A+E)
-- Qualifiers: teamID = 'TOR'
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_postseason_fielding AS
SELECT
    p.playerID,
    p.nameFirst,
    p.nameLast,
...
-- ============================================================
-- View 98: blue_jays_composite_player_value
-- Purpose: Composite player value score for Blue Jays hitters
...
-- Qualifiers: teamID = 'TOR', PA >= 1000
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_composite_player_value AS
WITH career AS (
    SELECT
        b.playerID,
        SUM(b.AB) + SUM(COALESCE(b.BB, 0)) + SUM(COALESCE(b.HBP, 0))
...
-- ============================================================
-- View 99: blue_jays_all_time_best_by_position
-- Purpose: Best Blue Jays at each position by career OPS
--          (min 200 G at that position for the Blue Jays).
-- Key formulas: OPS, games at position from fielding.
-- Qualifiers: teamID = 'TOR', G at position >= 200
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_all_time_best_by_position AS
WITH pos_games AS (
...
-- ============================================================
-- View 100: blue_jays_hall_of_fame_monitor
-- Purpose: HOF-relevant stats for Blue Jays players: career
...
--   and all-star data (Blue Jays stints only).
-- Qualifiers: PA >= 1000 OR IP >= 300 for the Blue Jays
-- ============================================================
CREATE OR REPLACE VIEW blue_jays_hall_of_fame_monitor AS
WITH bat_career AS (
    SELECT playerID,
...
pitch_career AS (
    ...
    FROM pitching WHERE teamID = 'TOR'
    GROUP BY playerID
),
awards AS (
    ...
    WHERE EXISTS (
        SELECT 1 FROM batting b WHERE b.playerID = ap.playerID
            AND b.yearID = ap.yearID AND b.teamID = 'TOR'
        UNION ALL
        SELECT 1 FROM pitching pi WHERE pi.playerID = ap.playerID
            AND pi.yearID = ap.yearID AND pi.teamID = 'TOR'
    )
    GROUP BY ap.playerID
),
allstars AS (
    SELECT playerID, COUNT(DISTINCT yearID) AS allstar_cnt
    FROM allstarfull WHERE teamID = 'TOR'
    GROUP BY playerID
),
hof AS (
    SELECT playerID,
